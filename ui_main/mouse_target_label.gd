...
##
## Uses [IVWorldController] and (if present) [IVFragmentIdentifier]. This node
## should be after (on top of) IVWorldControler but before (below) runtime GUI
## panels.[br][br]
##
...
func _ready() -> void:
	...
	IVStateManager.core_initialized.connect(_configure_after_core_inited)
