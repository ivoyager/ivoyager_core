...
The `/cubemaps/` directory holds every world map the simulator uses, stored as a six-face cube-face strip rather than an equirectangular image mainly to prevent polar artifacts. Each strip was reprojected by I, Voyager from an equirectangular source image and resampled to the stored face size.
...
- the source map's license, for maps from third-party sources (see [3RD_PARTY.md](3RD_PARTY.md) — e.g. the Björn Jónsson maps).
...
The `/bodies_2d/` directory contains one small flat-image icon per body (`<Body>.256.png`), used in the GUI. Each icon is rendered by I, Voyager from that body's 3D model or surface map; as a derivative work, each icon carries the same copyright and license as the source body asset it was rendered from:
...
They are documented as a group rather than individually; each inherits the license already documented for its source body.
