...
static func get_config_with_override(config_path: String, override_config_path: String,
		warn_conflict := true, section_prefix := "") -> ConfigFile:
	...
	if conflict_texts and warn_conflict:
		var warning := ("WARNING: The following sections or properties in '%s' are not present in"
			+ " '%s' and will be skipped. The absence could be due to a plugin update."
			% [override_config_path, config_path])
		if section_prefix:
			warning += " (This warning applies only to sections prefixed '%s'.)" % section_prefix
		warning += conflict_texts
