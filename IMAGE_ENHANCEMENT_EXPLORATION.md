...
while developing `shaders/surface.gdshader` (then named depixelation.gdshader) against the Moon. It exists to steer a
future effort, not to document current behavior. The only thing actually shipped is
`surface.gdshader`, whose de-pixelation core is bicubic resampling and nothing else.
...
- **Bicubic de-pixelation is the clear, cheap win** and is all `surface.gdshader`'s de-pixelation does.
