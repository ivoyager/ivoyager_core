...
* [Possibly project breaking] Per-shell spheroid-model configuration (cloud decks, atmospheric haze, limbs) has been taken out of the `shell<N>_*` body-table columns and moved to its own [shells.tsv](https://github.com/ivoyager/ivoyager_core/blob/master/tables/shells.tsv). Each body now lists its shells in a single `shells` field (`ARRAY[STRING]`, e.g. `SURFACE;CLOUDS;LIMB`); each tag names a row `SHELL_<body_name>_<tag>`. Per-shell transparency is now data (a `BaseMaterial3D.Transparency` column) rather than hard-coded as surface-opaque / overlay-alpha, and shadow-casting follows opacity instead of shell index — enabling e.g. a transparent cloud deck over an opaque sub-surface shell. Removed `IVAssetPreloader.max_shells` (shell count is now unbounded).
