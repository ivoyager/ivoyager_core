...
### Added
* [#18](https://github.com/ivoyager/ivoyager_core/issues/18) "Farwarp" compression keeps large and distant objects visible when zoomed in to small spacecraft. The camera near:far ratio is hard-capped at ~2^24 by float32 projection-plane math in the engine (see note in camera.gd), so anything beyond ~6 decades of the camera distance was frustum-culled — e.g., the Moon and Sun from the ISS, or Jupiter from Juno. Beyond a camera-relative start distance (IVCoreSettings `farwarp_start_ratio`, default 1e4× camera distance), visuals are now re-rendered along their true camera ray at logarithmically compressed distance and proportionally reduced scale, preserving screen direction and angular size *exactly*; the whole universe fits inside the existing far plane. True IVBody positions never change, so selection, GUI, camera targeting, lighting, and body mouse-picking are unaffected. New program node IVFarwarpManager drives the per-frame compression state (GDScript statics + `iv_farwarp_start` shader global) in a late pass at process priority +100 — after IVCamera has moved and origin-shifted the Universe, so world-space placements land in the frame's final coordinates (an earlier placement lags one frame of camera world-motion, which reads as violent shake on fast nearby orbiters and off-center models during fast camera rotation); IVBody applies the remap to its model space via an interposed top_level `farwarp_space` node positioned in world space from camera-relative math — float32 rounding then stays proportional to the compressed distance, where placement through the true-scale ancestor chain would jitter by whole ulps of the true distance (bodies below an angular-size cutoff keep true positions — their HUD symbols, which always follow the remap, represent them; stars bypass the cutoff); orbit lines and small-body points apply the same curve per-vertex in view space (`_farwarp.gdshaderinc`, new `farwarp_line.gdshader` base material replacing StandardMaterial3D on line visuals). Disable with IVCoreSettings `apply_farwarp` = false (rendering is then identical to previous behavior).
