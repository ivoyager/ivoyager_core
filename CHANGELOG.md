...
2. New shader globals `iv_exposure`, `iv_emission_energy_scale`, `iv_emission_luminance_scale` and `iv_limb_scale`. The Core editor plugin writes them into your project.godot from ivoyager_core.cfg on editor load. Shaders require these with or without physical lights; default values indicate to shaders that physical is inactive. 
...
* The atmosphere limb's glow (both the day term and forward scatter) is now gated by local sunlitness, so a night-side close-up no longer shows a lit annulus around the dark disc. The far backlit ring (inferior-conjunction geometry) sits at grazing sunlitness and survives at roughly half its former strength.
* Uranus and Neptune gain their own measured limb darkening, each fitted from its own disc in Irwin et al. (2024) rather than one inheriting the other's law on an argument from similarity — Uranus k = 0.788, Neptune k = 0.716. `band_pattern.gdshader` now carries `lunar_lambert` as well as `minnaert_k`, for the k < 1 direction Minnaert cannot serve. Levels re-solved for the new disc factors.
...
* `file_adjustments.tsv`: Arrokoth gains a `model_scale` of 10, its glb's own authored unit — it had no row, so a 36 km body was rendering 3 km long; its model is renamed `Arrokoth.1_10.glb`, carrying that scale in the file name as Eros and Hyperion already do. The dead rows for the five bodies that no longer ship a packed model (Mimas, Ceres, Charon, Iapetus, Phoebe) are removed. The unused `map_offset` column is removed along with its engine plumbing — an equirectangular map must be centred on the prime meridian, which every shipped map already is. Mimas now ships as a mesh plus albedo and normal cubemaps, like the other custom-mesh bodies, which is what lets its geometric albedo of 0.962 be stored at all.
