...
* `IVPhotometry` static class: the V-band anchors (magnitude-0 illuminance and 0 mag/arcsec² luminance) and the magnitude, illuminance and luminance conversions that the physical light system is built from.
...
* Body-table column `meter_albedo`: the albedo exposure metering divides by, where that is not the body's catalog `albedo`. The two agree on any body whose map is all there is to see, and part where a body's shells add light over it — metering knows only `albedo × illuminance / π`, so an atmosphere across the disc and a cloud deck above it return light it then over-exposes for. Earth fills it, at 0.30 against its cloudless 0.15, which brings its exposure down one stop; the column is empty on every other body and an empty cell changes nothing. This frees `albedo` to be only the asset-level target and the catalog value the UI reports, and Earth's went to Mallama's **0.434** with it — it had held the cloudless 0.15 only because one number had to serve both jobs, and it was the sole cell in the column that was not a published geometric albedo. Nothing renders differently for that half: measured in-app, the exposure moves 0.000000 EV. See [PHOTOMETRIC_MODEL.md](PHOTOMETRIC_MODEL.md).
* Glow (bloom) is on by default in `resources/ivoyager_environment.tres`, with every other glow property left at its engine default. It composites pre-tonemap in linear light and keys on `glow_hdr_threshold` (1.0), so under a `metering_key` of 0.5 the compensating camera decides what blooms: only what it has not exposed for — the sun, bright star cores, a blown atmosphere limb, overexposed cloud tops, clipped city cores, the near-opposition ring face — while a correctly metered surface sits below the threshold and does not. Because exposure acts before the threshold, a source blooms exactly when the camera is exposed such that it clips, and an eclipse dims its bloom with its light. The star field's PSF is not redundant with it and could not be: the PSF is the star's image and carries the photometry, while glow adds the wide scattering wings it has no way to, capped at `glow_hdr_luminance_cap` so bloom is proportional to flux only between threshold and cap and every brighter star blooms at the cap, differing by footprint alone. A new *Glow: the bloom pass* section in [PHOTOMETRIC_MODEL.md](PHOTOMETRIC_MODEL.md) documents which of Godot's glow properties a project may change and which carry the contract (`glow_bloom` must stay 0.0; `glow_hdr_threshold` must not come down; Soft Light is the one wrong blend mode, being applied after tonemapping), and records what the Compatibility renderer does differently. Glow is now OFF there (`IVWorldEnvironment._ready()`, beside the `adjustment_enabled` gate it joins), because measured it is a pure loss: the glow buffers inherit the scene buffer's RGB10_A2 and store `0.25 x color`, so the per-texel feed clamps at 4.0 whatever `glow_hdr_luminance_cap` says — that property and `glow_levels` are byte-for-byte inert and the far sun's halo is the same 4 px with the pass on as with it off — while enabling it moves tonemapping into a post pass that re-runs the transfer bracket `display_write()` pre-inverts exactly once, measured at **0.041x on 6-8 code content, 0.19x at 8-10, 0.66x at 12-18 and 0.84x over the whole frame** against Forward+'s 1.000x at every level. That is the Milky Way and the faint stars. With it off the two renderers agree on the same frame to 0.8 %, and four parity views that had been 21 % dark on Compatibility (Callisto, Mimas, Mimas' lit disc, Saturn) come back to 0.99-1.01. The wings a bright source needs are drawn in the shader instead — see the star glare entry. A project that wants the pass on Compatibility anyway can author glow into its own Environment.
* Every star carries its GLARE: the wide `r^-2` wings its Gaussian core does not have, drawn in the shader rather than left to the engine's glow pass (`star_glare_*` in `shaders/_star_point.gdshaderinc`, shared by the catalog field and by an in-scene star through one `IVStarSettings`). The pass could not carry them: its per-texel feed is capped, so bloom stops being proportional to flux above the cap — the sun runs V -26.7 at Earth to -19.0 at Pluto, a factor 1225, and its bloomed halo moved only 39 -> 29 px, one doubling per 19 magnitudes, against the brightest field star's 2 px — and on Compatibility the cap is a buffer format and there is no halo at all. So the far sun read as an ordinary bright star. In the shader the intensity is still a float32 that nothing has clamped, and the result lands identically under both renderers: the halo's 32-code radius at Pluto goes 29 -> 44 px on Forward+ and **5 -> 54 px on Compatibility**. The wing's SHAPE is physical (a real PSF is a power law outside its core, exponent ~2 for an eye or a lens); its amplitude cannot be, since the CIE glare law's ~10 % of a source's light is about 4500x saturation over the whole frame at the sun's flux and a dark-adapted exposure — so it carries a compression exponent, `glare_gamma`, as `intensity_gamma` already does for the field, and `glare_scale` is anchored so the far sun reproduces the Forward+ glow halo it replaces. New `IVStarSettings` values `glare_scale`, `glare_gamma` and `glare_max_px`, exported on `IVStarsVisual` beside the rest of the star camera; `glare_scale` 0 turns the glare off. The field's wing rides its own point sprites (2-6 px, and inside the core for anything fainter than about V 3); an in-scene star's runs to hundreds of px and rides a quad instead, the new `shaders/sun_glare.gdshader`, since POINT_SIZE's driver maximum is as low as 1 in the GLES3 spec. Costs about a tenth of a millisecond: median GPU frame time 1.339 -> 1.438 ms on Forward+ and 1.589 -> 1.593 ms on Compatibility, most of it the field rather than the sun, since a wing widens every star's sprite (a V 6.5 star 3.7 -> 8.7 px, Sirius 5.3 -> 24.9). Sirius and Vega now exceed the 20 px `stars.gdshader` calls known-good for POINT_SIZE across platforms; a driver clamp would truncate the wing's faint outer part and nothing else, and `glare_scale` or `glare_max_px` is the lever if one ever bites. See [PHOTOMETRIC_MODEL.md](PHOTOMETRIC_MODEL.md).
...
* `VISUAL_MODEL.md` — the spatial companion to `PHOTOMETRIC_MODEL.md`: how a double-precision simulation renders through a float32 pipeline (parenting cancellation, origin shifting, farwarp compression, the ~2^24 near:far ceiling), the analytic sun-occlusion system and the local shadow maps, culling and lifecycle, the orbit-line tiers and render-frame pin, point fields, mouse picking, and a TODO recording the known gaps a code audit surfaced (high-speed registration loss, missing planetary point sources, analytic-shadow receiver gaps, a middle-domain occlusion double-count).
...
* The air in front of a disc is composited by the disc's own shaders, not blended over them. The limb shell had drawn the whole disc branch — the veil, the twilight glow, the far half of an optical-limb ray — as a premultiplied blend over whatever the surface and cloud shells had rendered. That is an identity only where the blend runs in linear light, and the Compatibility renderer blends display-referred and clamped: a faint veil over a lit disc landed crushed there, hardest where faintest, which cost Earth's lit side its atmospheric softening entirely and Titan its lit-side haze (radial luma vs Forward+, disc core / outer disc: Earth 0.66 / 0.48, Mars 0.69 / 0.55). Each surface, band and cloud fragment now evaluates the new `atm_disc_air()` for its own ray and sums the composite in linear before its own display conversion — the path added over everything it renders, the luma of the disc's view transmittance multiplied into all of it (specular as its square root), `atm_view_tint` carrying the chromatic complement exactly as before. In linear light the relocation is an algebraic identity (the path distributes through a deck's alpha mix at weight `α + (1 − α) = 1`): Forward+ re-renders every airless view BIT-IDENTICALLY, and the atmosphere views to within a few dozen silhouette-staircase pixels — the air now follows the rendered facet's geometry rather than the mathematical sphere's — plus a ≤2-code lit-side sheen where the specular tail's f90 clamp escapes an F0-only scale, the same limit the eclipse shadow accepts. Compatibility recovers the disc: core / outer after, Earth 0.97 / 0.94, Mars 0.97 / 0.89, Venus 0.99 / 0.98, Titan's core at 1.00. The beyond-limb RING is recovered in the same pass: the limb shell had handed its linear radiance to the engine's own conversion raw, crushed hardest where faintest (0.03–0.22 of Forward+, effectively absent — Titan rendered with no limb at all), so on the display-referred branch it now self-lights on `sun_light_energy` and writes its finished value, the rings shell's restatement over the pedestal that is the provable constant 0 — and with `blend_premul_alpha` the colour is the whole premultiplied term, so plain `display_write()` restates it with no `display_mix` pair. Ring-annulus luma vs Forward+ goes Earth 0.40 → 1.02, Mars 0.18 → 0.92, Venus 0.59 → 0.98, Titan 0.07 → 0.79 (diluted by the annulus fit on a crescent; rendered, Titan's haze ring stands where its limb was absent), with Forward+ bit-identical across all eighteen posed views — the linear branch is untouched. At the very silhouette the two owners hand off over a thin feathered band (`ATM_RIM_HANDOFF`), because no polygonal mesh covers its own mathematical silhouette — a hard partition measured as a dotted arc of ~270 dark pixels. `atm_limb()` gains a `disc_hit` out and the limb shader discards below the band; a project shader calling it must add the argument. The new gate `atm_present()` also ends a cost that predates this change: `IVShellsModel` feeds the geometry uniforms to every shader shell, so every airless body had been running the atmosphere functions' arithmetic against zero optical depth — and that zero-tau evaluation, moved into the surface shaders' own compilation, emitted visible driver garbage (glow-bloomed beads along Saturn's silhouette) until gated. The 2D icon rig now feeds `sun_light_energy` from its key light, which the veil and the twilight term both ride; the four atmosphere-body icons will shift slightly at their next deliberate re-capture, their twilight having previously run at the shader default of 1.

### Fixed
* Under Forward+, the four atmosphere bodies flashed bright point sources at their lit-side silhouettes as they turned — a bloomed blob with a black pixel at its core, appearing and vanishing over a small rotation. A NaN out of the disc quadrature at grazing incidence, where its intermediates span tens of orders of magnitude (a Chapman column clamping at e^60 against an extinction that underflows to exactly zero, whose product is a NaN); measured at about one fragment per two dozen views, always at the silhouette and on the lit side. One such fragment is not a local defect, because the glow pass smears it across a neighbourhood — and it showed under Forward+ only because the display-referred path's own `max(value, 0.0)` scrubbed it before it could bloom. It became visible when the disc's air moved into the disc shaders (see the disc-air entry under Changed), which sample `mu` from the MESH — a facet at the silhouette presents `mu` at or below zero — where the limb shell's ray-sphere intersection could not. `atm_disc()` now ends with a finite guard; it cannot be attributed to a term, being sensitive to the surrounding compilation (`isnan()` reads inside the loop stop it happening, while the same reads one scope out catch it without suppressing it — the same driver behaviour `atm_present()` was added for). Two real numerical corrections came with it: the ray's tangent altitude is formed as `-R mu^2 / (1 + sqrt(1 - mu^2))` rather than `b_ray - R`, which is the difference of two planetary radii and collapses in float32 to exactly zero for a grazing ray, putting a quadrature node on the ray's own tangent point where the Jacobian is infinite; and `atm_disc_air()` decides the handoff fade before running the quadrature, so the degenerate ray the limb shell owns outright is never evaluated. Airless bodies re-render bit-identically and the atmosphere bodies move only on the guarded fragments and a few last-bit roundings.
* The fragment-id broadcast bloomed into a blob at the cursor once glow was on, worst exactly where mouse-over identification matters most — a crowded asteroid field, where many points fall inside the probe's 49-pixel grid at once. An id-bearing fragment wrote its raw encoded channels, up to 2048, into the same buffer the glow pass reads, so every grid pixel bloomed at the cap: measured hovering an orbit line, 553 of the 625 pixels around the cursor were saturated. The broadcast now goes through `id_broadcast()` in `shaders/_fragment_id.gdshaderinc`, which carries each channel in [0.5, 1.0] — topping out exactly at the glow threshold, where the pass's smoothstep still returns zero — and the probe compute shaders lift it back out. Same box measures 31 of 625 saturated, with hover picking unchanged (same hits on the same pixels, verified in-app). This costs a channel one bit, so an id is 30 bits rather than 33 (Project Note 6): the band is one binade, whose half-float step is 1/2048, and a broadcast that does not survive RGBA16F storage exactly would decode as the wrong id. It also narrows the probe's accept window from "every channel above 0.5" to "every channel inside one octave", so content brighter than an id — a star, a lit limb — can no longer be mistaken for one. Picking itself was never at risk from glow: the probe reads at `POST_TRANSPARENT`, before the glow pass.
* Bodies rendered wrong under the Compatibility renderer (including the web export): high-albedo moons blew out to flat white — 89 % of Enceladus' disc, 64 % of Mimas', 35 % of Venus', 11 % of untagged Ganymede's — and Saturn came out 12 % over-saturated. That renderer is display-referred at both ends of a shader where Forward+ and Mobile are linear at both: a `source_color` texture arrives still encoded, and what a shader writes is taken as encoded too. The two conventions agree for a value that is only sampled and multiplied by light — an sRGB byte of 128 round-trips to 128 under both renderers — and agree for nothing else: not for arithmetic on a sampled colour, where a *linear* coefficient (a range tag's affine unpack, a disc-photometry factor, a band tint, `albedo_scale`, a ring's phase boost) applied to a display-referred value makes a multiplier `m` act like `m^2.4`, and not for a computed radiance (an emission map in cd/m², a star's flux), which nothing samples and so nothing cancels. New `shaders/_display.gdshaderinc` is the fix and the only place that decides any of it: a shader decodes what it samples, does its colour arithmetic in linear, and writes through new `display_write()` / `display_write_albedo()`, which pre-invert the engine's own approximate transfer pair — a polynomial decode and a no-linear-segment power encode bracket everything a fragment writes, crushing dim content (a written 0.05 landed at 0.030) — so the framebuffer lands the exact-sRGB value Forward+ lands, with no upper clamp (a range-tagged albedo above 1.0 must survive the light multiply) and with zero writing exact zero on the slot the renderer decodes and then multiplies (an eclipsed albedo of 0 written with the dark boost had rendered Mimas in Saturn's shadow as a solid white disc at a dark-adapted exposure). Everything is gated by the new `iv_display_encode` global that `IVGraphicsManager` writes once at startup; every conversion is the identity on a renderer that handles its own colour space, so Forward+ and Mobile render bit-identically to before. The `compat_albedo_shadow` fallback also left SPECULAR unshadowed (AO is ambient-only on this renderer), so the occluder shadow now rides SPECULAR as the square root. The background panorama takes the same treatment — the GLES3 sky pass expects display-referred COLOR exactly as its scene pass expects display-referred ALBEDO — so `starmap_background.gdshader` decodes its sample and encodes its output. And Saturn's rings, dimmed to 0.73x by the alpha blend running on display-referred values, self-light on the Compatibility branch (Lambert, which the end-to-end measurement favors decisively at the ring's near-equinox grazing geometry) and restate their (colour, alpha) pair in the framebuffer's own space (new `display_mix()`, over the empty sky the ring actually stands on; `IVSunOcclusionManager` feeds the new `sun_light_energy` uniform, published per star by `IVDynamicLight.star_light_energies`); the `IVRings` Compatibility boost overrides (1.25/1.5 against the 3.0/2.0 every other renderer uses) and their metering constant are retired with that, having been tuned against the old broken pipeline. Measured against Forward+ over the lit disc: every airless body lands within 1 % (1.00–1.01, against 1.11–1.73) at any exposure, the blowouts are gone (Mimas 64.3 % → 0.3 %, against Forward+'s own 0.2 %), the sky sits at 1.00x at its median and p90, and the lit ring face at 0.99x from 0.73x. What is deliberately NOT corrected is the blend itself, which runs after a shader returns and therefore on display-referred values, where a sum is not a sum. The largest costs fell on the atmosphere — since recovered entirely (see the disc-air entry under Changed: the disc's air left the blend, and the beyond-limb ring restates). What remains at the blend: a bright cloud deck cannot fully whiten at partial coverage, and a terminator's last codes cut slightly early. *Renderer parity* in [PHOTOMETRIC_MODEL.md](PHOTOMETRIC_MODEL.md) carries the measurements, records why a fragment must not instead estimate what its blend lands on, and lists what a targeted patch could still recover. Forward+ measured byte-identical throughout.
