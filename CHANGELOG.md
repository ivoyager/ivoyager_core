...
### Added
* Analytic sun-occlusion system replaces shadow maps for all astronomical-scale shadows: ring shadows on bodies (individual ringlet detail, physical penumbra, no banding — sampled from the full-resolution ring alpha profile instead of a shadow map that can't carry it), body shadows on rings (correctly oblate), and eclipses/transits as exact sun-disc occlusion by oblate spheroids (penumbra/antumbra fall out of the math; e.g. the 2015-01-24 multiple shadow transit renders on Jupiter with per-moon penumbra widths). New program node IVSunOcclusionManager feeds per-frame uniforms to receiving shaders (a shader opts in by declaring the `occluder_data_a` uniform; see `shaders/_sun_occlusion.gdshaderinc`) and publishes a camera-point sun-visible fraction that dims the local lights (new `apply_sun_occlusion` column in dynamic_lights.tsv) — craft-scale objects get eclipse and ring shadows as uniform dimming (the ISS goes dark through Earth's umbra with the physical penumbra ramp). CPU mirrors of the shader math are in IVAstronomy. Works with the Compatibility renderer via an albedo fallback (AO never reaches direct light there). Opt-out: IVCoreSettings `apply_analytic_shadows`.
* Shadow-map hygiene under farwarp: every light's shadow reach is clamped to the farwarp start (a shadow map can't be correct across the warp boundary — near casters stamped oversized shadows on warp-compressed bodies, and a warped body's light-space imprint false-shadowed its camera-space self, e.g. Earth's daylit disc rendering near-black behind the ISS). Large bodies cast into the local (near/middle) shadow maps only while inside the farwarp start and IVCoreSettings `local_shadow_caster_ceiling` (granted per frame in IVBody.update_farwarp), so nearby "terrain" still shadows landers while astronomically-distant geometry stays out of the maps.
* Godot directional (local-scene) shadows — e.g. spacecraft self-shadowing — now work under the Compatibility renderer, using the same shadowed multi-light setup as Forward+. Controlled by new IVCoreSettings `apply_gl_compatibility_shadows` (default true); set false to restore the single unshadowed light. Previously disabled outright for Compatibility-renderer defects (cull/caster-mask handling, multi-light energy, and shadows shifting color — godotengine/godot#90259); those did not reproduce in Godot 4.7 testing, but the fallback remains for targets where they might (notably some web/GPU combinations). Independent of the analytic astronomical shadows above.
...
### Changed
* [shader/gdshaderinc breaking] Renamed `depixelation.gdshader` → `spheroid_surface.gdshader` and `cloud_detail.gdshader` → `cloud_shell.gdshader` (IVGlobal.resources keys `spheroid_surface_shader`, `cloud_shell_shader`; spheroids.tsv/shells.tsv cells updated). The old names described one technique each; the files are the standard shell materials and now also host the analytic sun-occlusion terms.
* [Project breaking] dynamic_lights.tsv restructured: the 4 semi-opaque far lights collapse into one unshadowed `SUN_FAR` row (the far light has no shadow map — astronomical shadows are analytic); `shadow_caster_mask` cells are plain INT (`0b100000000` = the new LOCAL_SHADOW_CASTER bit); `shadow_opacity` column removed (with the out-of-documented-range values > 1.0); new BOOL column `apply_sun_occlusion`. Directional shadow atlas pressure drops from 6 shadowed lights to 2, so near/middle shadow resolution improves for free.
* [Project breaking] rings.tsv columns `shadow_lod` and `shadow_radial_noise_a/b/c` removed with the caster rig.
...
### Removed
* [Project breaking] `IVGlobal.ShadowMask` enum (replaced by the single `IVGlobal.LOCAL_SHADOW_CASTER` layer bit), `rings_shadow_caster.gdshader` + resource key `rings_shadow_caster_shader`, `IVRingsShadowCaster` (inner class of IVRings), `IVAssetPreloader.get_rings_shadow_caster_texture()` and `get_blue_noise_1024()` (the rings caster was the only blue-noise consumer; the asset remains in ivoyager_assets).
...
* False shadows under farwarp (e.g., a spacecraft's dish stamping a visible shadow on the planet behind it, or a planet's daylit disc false-shadowed to black when viewed from a nearby craft). See the shadow-map hygiene entry under Added.
