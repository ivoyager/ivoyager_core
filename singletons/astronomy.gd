...
# *****************************************************************************
# Photometry. Visible-light anchors and conversions: apparent V magnitude is
# the flux anchor (m = 0 defined as MAG0_ILLUMINANCE at the observer) and
# mag/arcsec^2 is the surface-brightness anchor. IVExposureManager builds the
# physical-light chain from these.
...
func get_apparent_magnitude(absolute_magnitude: float, distance: float) -> float:
...
func get_illuminance_from_apparent_magnitude(apparent_magnitude: float) -> float:
	return MAG0_ILLUMINANCE * 10.0 ** (-0.4 * apparent_magnitude)


## Returns luminance (internal units) for [param surface_brightness]
## (V mag/arcsec^2).
func get_luminance_from_surface_brightness(surface_brightness: float) -> float:
...
func get_star_disc_luminance(absolute_magnitude: float, radius: float) -> float:
